...
import com.cognizant.seller.cmd.api.user.commands.RemoveUserCommand;
...
public class RemoveProductController {
    ...
    public ResponseEntity<BaseResponse> removeProduct(@PathVariable(value = "id") String id) {
        try {
            commandGateway.send(new RemoveProductCommand(id));

            return new ResponseEntity<>(new BaseResponse("Product removed successfully!"), HttpStatus.OK);
        } catch (Exception e) {
            var safeErrorMessage = "Error while processing remove product request for id - " + id;
